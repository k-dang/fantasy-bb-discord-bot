...
const { upsertAuthentication } = require('./db/authRepository');
...
app.get('/api/callback', async (req, res) => {
  const tokenSet = await yahooOidc.getTokenSet(req);

  const encryptedAccessToken = encrypt(tokenSet.access_token);
  const encryptedRefreshToken = encrypt(tokenSet.refresh_token);
  // store the tokens
  await upsertAuthentication(encryptedAccessToken, encryptedRefreshToken);
