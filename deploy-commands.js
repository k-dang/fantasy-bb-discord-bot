...
const isGlobalDeploy = args.length > 0 && args[0] === 'global';
...
for (const folder of commandFolders) {
  ...
  for (const file of commandFiles) {
    ...
    if ('data' in command && 'execute' in command) {
      // skip reload command for global deployment (only for local deployment)
      if (isGlobalDeploy && command.data.toJSON()['name'] === 'reload') {
        continue;
      }
